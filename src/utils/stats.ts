...
const getMapLeaderboard = (matches: Match[]): MapScore[] => {
  ...
  matches.forEach((match: Match) => {
    ...
    if (!findMatch) {
      mapLeaderboard.push({
        ...
        getWinrate: function () {
          return parseInt(
            Number((this.wins * 100) / (this.wins + this.losses)).toFixed(2)
          );
        },
      });
    } else {
  ...
  const sortedMapLeaderboard = mapLeaderboard.sort(
    (a: MapScore, b: MapScore) => {
...
const getRoleLeaderboard = (matches: Match[]): RoleScore[] => {
  ...
  const sortedRoleLeaderboard = roleLeaderboard.sort(
...
const getHeroLeaderboard = (matches: Match[]): HeroScore[] => {
  ...
  const sortedHeroLeaderboard = heroLeaderboard.sort(
